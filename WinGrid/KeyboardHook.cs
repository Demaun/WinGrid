...
namespace WinGrid
{
	...
	{
		const int keyUp = PInvoke.KeyStateConstants.WM_KEYUP;
		const int systemKeyUp = PInvoke.KeyStateConstants.WM_SYSKEYUP;
		const int keyDown = PInvoke.KeyStateConstants.WM_KEYDOWN;
		const int systemKeyDown = PInvoke.KeyStateConstants.WM_SYSKEYDOWN;

		static readonly HashSet<KeyCombination> currentHookKeys = new HashSet<KeyCombination>();
		static readonly PInvoke.LowLevelKeyboardProc lockdownHookCallBack = LockdownHookCallBack;
		...
		bool isDisposed;
		bool keyReleased;
		readonly PInvoke.LowLevelKeyboardProc hookCallback;
		...
		{
			lockdownHookId = PInvoke.SetWindowsHookEx(
				PInvoke.KeyStateConstants.WH_KEYBOARD_LL,
		...
		public static void ReleaseFullKeyboardLockdown()
		{
			if (lockdownHookId != IntPtr.Zero)
			{
				PInvoke.UnhookWindowsHookEx(lockdownHookId);
		...
		static bool IsKeyPressed(KeysEx key)
		{
			...
			const int keyPressed = PInvoke.KeyStateConstants.KEY_PRESSED;
			return (PInvoke.GetKeyState((int)key) & keyPressed) == keyPressed;
		}
